...
func TestPoolSelector_LeastConnPicksFewestRequests(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if p.ID != 2 {
			t.Fatalf("least_conn: got proxy %d, want 2 (fewest requests)", p.ID)
		}
	}
}
...
func TestPoolSelector_LeastConnSkipsCooled(t *testing.T) {
	...
	defer cd.Stop()
	ps := newMethodSelector("least_conn", px(1, 5), px(2, 10), px(3, 20))
	ps.domainCD = cd
	// Proxy 1 has the fewest requests but is cooled for foo.com → expect proxy 2.
